...
from Queu.models import tickets, customers, tellers, service, logs, counters
...
def create_ticket(request):
    ...
    """
    msisdn = request.data['customer_msisdn']
    ...
    Ticket_number = ticket_no.ticket
    print(ticketing.teller_id)
    Ticket_message= "Hello " + Customer_name + ", Your Ticket Number is: "+str(Ticket_number)+"."
    payload = "{\"ticket_message\": \"" + Ticket_message +"\"}"
    url = "https://usque.serveo.net/send_text/"+ str(msisdn)
    headers = {'Content-Type':'application/json'    }
    requests.get(url = url,data=payload ,headers=headers)
...
def cancel_ticket(request):    
    ...
    try:
        if request.method == 'GET':
            ...
            ticket = tickets.objects.filter(teller_id=request.data['teller_id']).get(ticket=no)
            customer = customers.objects.get(id=ticket.customer_id)
            delete=tickets.objects.filter(teller_id=request.data['teller_id']).get(ticket=no).delete()
            data={
                "message":"ticket deleted",
                "status_code":200
            }
            url = "https://usque.serveo.net/lcd/"+ str(customer.fname) +"/"+ str(no)
            headers = {'Content-Type':'application/json'}
